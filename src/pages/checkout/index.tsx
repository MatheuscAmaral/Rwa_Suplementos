...
export const Checkout = ({ className, ...props }: CardProps) => {
    ...
    const { cart, total, addItemCart, removeItemCart, descontos, clearAll } = useContext(CartContext);
        ...
        <main>
           {
             pedidoMessage != "" ? (
                <div className="flex flex-col gap-5 h-svh items-center mt-52">
                    ...
                    <Link to={"/pedidos"} className="bg-blue-700 text-white p-3 rounded-md">Ver pedidos</Link>
                </div>
             ) : (
                ...
                <section className="w-full md:max-w-80 rounded-lg px-5 py-4 pb-5 border">
                    ...
                    <p className="mt-3 text-xs flex justify-between font-medium text-gray-500">SubTotal
                        <span>
                            {
                            total <= 0 ? getTotal(cart[0].price) : getTotal(total + descontos)
                            }
                        </span>
                    </p>
                    ...
                    <p className="mt-2 text-sm flex justify-between font-semibold">Total 
                        <span>
                            {
                                total <= 0 ? getTotal(cart[0].priceWithDiscount > 0 ? cart[0].priceWithDiscount : cart[0].price) : getTotal(total)
                            }
                        </span>
                    </p>
                    ...
                    {
                        cart.length > 0 ? (
                        <button id='button' onClick={() => finalizarPedido()} className={`${loading ? "disabled cursor-not-allowed opacity-70" : ""} text-sm bg-blue-800 text-white flex items-center justify-center py-3 w-full rounded-lg border-0 hover:bg-blue-700 transition-all  mb-3`}>
                        {
                            loading ? (
                    ...
                    <Card className={`${cn("w-full", className)} ${openModal && address ? "block" : "hidden"} relative mx-auto max-w-lg`} style={{ height: 530 }} {...props}>
                        ...
                        <CardContent className="flex flex-col">
                            ...
                            <div className="flex flex-col gap-2 text-sm text-gray-700">
                                    ...
                                    <Input id="cep" value={cep} onChange={(e) => getCep(e.target.value)} required type="number" placeholder="Digite o seu cep..."/>
                            </div>
                        ...
                        <CardFooter>
                            <Button type="submit" onClick={() => saveEndereco()} className={`w-full h-11 bg-blue-700`} >
                                {
